  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      ...
      <?php
        ...
        $keywords = $_GET['keywords'];
        @$sql1 = "SELECT count(*) FROM orders where order_num like '%{$keywords}%' or tel like '%{$keywords}%' address like '%{$keywords}%' name like '%{$keywords}%' or status='{$status}'";
        $result1 = mysql_query($sql1);
        $count = mysql_result($result1,0,0);
        @$sql = "SELECT * FROM orders where order_num like '%{$keywords}%' or tel like '%{$keywords}%' address like '%{$keywords}%' name like '%{$keywords}%' status='{$status}'";
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'"
        ...
        <td align="center" valign="middle" class="borderright borderbottom">{$a['gid']}</td>
